...
除非有什么特别的规定，否则，一个 recipient **MAY** 尝试从一个非法的结构中恢复可用的协议元素。HTTP 并没有定义特别的错误处理机制除非它对安全性有着直接的冲击，因为不同的应用程序可能需要不同的错误处理策略。比如，一个 Web 浏览器可能想要正大光明地恢复一个定位头字段没有通过 ABNF 解析的 Response，但一个系统控制客户端可能认为任何形式错误恢复都是危险的。
