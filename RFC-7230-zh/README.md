...
当一个报文没有 `Transfer-Encoding` 头字段时，一个 `Content-Length` 头字段可以为潜在的有效载荷体提供可预期的大小，以一个十进制数表示的八位字节个数。对于的确包含有效载荷的报文来说，`Content-Length` 字段值提供了必要的解包信息以决定载荷主体（以及整个报文）在什么位置结束。对于不含有效载荷主体的报文来说
